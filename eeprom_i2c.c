...
struct EepromI2cChipConfig_t {
  ...
  uint8_t pageSize;   /**< Page buffer size in bytes; writes must not cross page
                         boundaries. */
  uint8_t writeCycleMs; /**< Internal write cycle time in milliseconds (tWR from
                           datasheet). */
  uint16_t addrSize;    /**< Word address width: I2C_MEMADD_SIZE_8BIT or
                           I2C_MEMADD_SIZE_16BIT. */
};

/** @brief Read-only chip configuration table stored in flash.
 *         Only entries for enabled EEPROM_I2C_SUPPORT_xxx chips are compiled
 * in. */
...
/**
 * @brief Calculate a CRC-16-Modbus checksum (CRC-16-ANSI, polynomial 0x8005
 * reflected).
 *
...
/**
 * @brief Return the number of upper address bits embedded in the I2C device
 * address byte.
 *
 ...
 * 8-bit word address field. Instead, the upper address bits are multiplexed
 * into the A0/A1/A2 positions (bits 3:1) of the I2C device address byte,
 * effectively creating 256-byte "banks".
 *
...
/**
 * @brief Compute the effective I2C device address for a given absolute memory
 * address.
 *
 ...
 * unchanged.
 *
 * @param handle    Pointer to the EEPROM handle (provides base i2cAddr and
 * chipConfig).
 ...
 *
 * @return HAL_OK if all bytes were read successfully, or a HAL error code on
 * failure.
 */
static HAL_StatusTypeDef I2C_ReadBytes(EepromI2cHandle_t *handle,
                                       uint16_t memAddr, uint8_t *pData,
                                       uint16_t size) {
  if (GetDevAddrBits(handle->chipConfig) == 0) {
    return HAL_I2C_Mem_Read(handle->hi2c, handle->i2cAddr, memAddr,
                            handle->chipConfig->addrSize, pData, size,
                            HAL_MAX_DELAY);
  }
  ...
  while (bytesRead < size && status == HAL_OK) {
    ...
    status = HAL_I2C_Mem_Read(handle->hi2c, devAddr, bankOffset,
                              I2C_MEMADD_SIZE_8BIT, &pData[bytesRead],
                              bytesToRead, HAL_MAX_DELAY);
 ...
 *
 * @param handle  Pointer to the EEPROM handle.
 * @return GPIO_PIN_SET or GPIO_PIN_RESET that must be written to WpPin to lock
 * WP.
 */
...
/**
 * @brief Return the GPIO state that de-asserts (unlocks) the Write Protect
 * signal.
 *
 ...
 *
 * @param handle  Pointer to the EEPROM handle.
 * @return GPIO_PIN_SET or GPIO_PIN_RESET that must be written to WpPin to
 * unlock WP.
 */
 ...
 *
 * The transfer is split at page boundaries to comply with the chip's page
 * buffer limit. After each page write, the function waits for the internal
 * write cycle (tWR) using HAL_Delay() before issuing the next write. The Write
 * Protect GPIO is de-asserted at the start and re-asserted after the last page.
 *
 ...
 *         on failure.
 */
static HAL_StatusTypeDef I2C_WriteBytes(EepromI2cHandle_t *handle,
                                        uint16_t memAddr, const uint8_t *pData,
                                        uint16_t size) {
  HAL_StatusTypeDef status = HAL_OK;
  ...
  while (bytesWritten < size && status == HAL_OK) {
    ...
    status = HAL_I2C_Mem_Write(
        handle->hi2c, devAddr, wordAddr, handle->chipConfig->addrSize,
        (uint8_t *)&pData[bytesWritten], bytesToWrite, HAL_MAX_DELAY);
...
void EEPROM_I2C_Init(EepromI2cHandle_t *handle, I2C_HandleTypeDef *hi2c,
  ...
  handle->maxDataSize =
      handle->chipConfig->totalSize - startAddr - sizeof(EepromI2cHeader_t);
...
EepromI2cStatus_t EEPROM_I2C_Write(EepromI2cHandle_t *handle, const void *pData,
  ...
  if (HAL_OK == I2C_ReadBytes(handle, handle->startAddr,
                              (uint8_t *)&existingHeader,
                              sizeof(EepromI2cHeader_t))) {
    if (existingHeader.DataLength == dataLength &&
        existingHeader.Crc16 == newCrc) {
      if (dataLength > EEPROM_I2C_MAX_BUFFER_SIZE)
        return EEPROM_I2C_ERR_SIZE;

      uint8_t tempBuffer[EEPROM_I2C_MAX_BUFFER_SIZE];

      if (HAL_OK == I2C_ReadBytes(handle,
                                  handle->startAddr + sizeof(EepromI2cHeader_t),
  ...
  if (HAL_OK != I2C_WriteBytes(handle, handle->startAddr, (uint8_t *)&header,
                               sizeof(EepromI2cHeader_t)))
  ...
  // ========================================================================
  if (HAL_OK != I2C_WriteBytes(handle,
                               handle->startAddr + sizeof(EepromI2cHeader_t),
...
EepromI2cStatus_t EEPROM_I2C_Read(EepromI2cHandle_t *handle, void *pData,
  ...
  if (HAL_OK != I2C_ReadBytes(handle, handle->startAddr, (uint8_t *)&header,
                              sizeof(EepromI2cHeader_t)))
    return EEPROM_I2C_ERR_I2C;
