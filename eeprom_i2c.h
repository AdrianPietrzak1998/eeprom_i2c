...
#if !defined(EEPROM_I2C_SUPPORT_M24C01) &&                                     \
    !defined(EEPROM_I2C_SUPPORT_M24C02) &&                                     \
    !defined(EEPROM_I2C_SUPPORT_M24C04) &&                                     \
    !defined(EEPROM_I2C_SUPPORT_M24C08) &&                                     \
    !defined(EEPROM_I2C_SUPPORT_M24C16) &&                                     \
    !defined(EEPROM_I2C_SUPPORT_M24C32) &&                                     \
    !defined(EEPROM_I2C_SUPPORT_M24C64) &&                                     \
    !defined(EEPROM_I2C_SUPPORT_M24C128) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_M24C256) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_M24C512) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_AT24C01) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_AT24C02) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_AT24C04) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_AT24C08) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_AT24C16) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_AT24C32) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_AT24C64) &&                                    \
    !defined(EEPROM_I2C_SUPPORT_AT24C128) &&                                   \
    !defined(EEPROM_I2C_SUPPORT_AT24C256) &&                                   \
    !defined(EEPROM_I2C_SUPPORT_AT24C512)
...
// --- Microchip/Atmel AT24Cxx series ---
  ...
  EEPROM_I2C_CHIP_AT24C04, /**< Microchip AT24C04 - 512 B, 16 B page, 8-bit addr
                              (banked) */
#endif
#ifdef EEPROM_I2C_SUPPORT_AT24C08
  EEPROM_I2C_CHIP_AT24C08, /**< Microchip AT24C08 - 1 KB, 16 B page, 8-bit addr
                              (banked) */
#endif
#ifdef EEPROM_I2C_SUPPORT_AT24C16
  EEPROM_I2C_CHIP_AT24C16, /**< Microchip AT24C16 - 2 KB, 16 B page, 8-bit addr
                              (banked) */
#endif
  ...
  EEPROM_I2C_CHIP_AT24C128, /**< Microchip AT24C128 - 16 KB, 64 B page, 16-bit
                               addr */
#endif
#ifdef EEPROM_I2C_SUPPORT_AT24C256
  EEPROM_I2C_CHIP_AT24C256, /**< Microchip AT24C256 - 32 KB, 64 B page, 16-bit
                               addr */
#endif
#ifdef EEPROM_I2C_SUPPORT_AT24C512
  EEPROM_I2C_CHIP_AT24C512, /**< Microchip AT24C512 - 64 KB, 128 B page, 16-bit
                               addr */
#endif
} EepromI2cChipType_t;
...
/**
 * @brief Maximum size (in bytes) of the internal verification/comparison
 * buffer.
 *
...
typedef enum {
  ...
  EEPROM_I2C_ERR_IDENTICAL, /**< Write skipped - stored data is identical to new
                               data (wear leveling). Not an error; treat as
                               EEPROM_I2C_OK if unchanged data is acceptable. */
...
/** @brief Forward declaration of the private chip configuration structure
 * (defined in .c). */
...
typedef struct {
  I2C_HandleTypeDef
      *hi2c;            /**< Pointer to the STM32 HAL I2C peripheral handle. */
  ...
  uint8_t wpInverted;   /**< Write protect polarity.
                             0 = WP active LOW (standard - pin HIGH disables
                           write).   1 = WP active HIGH (e.g. NPN transistor - pin
                           LOW disables write). */
  uint8_t i2cAddr; /**< I2C device address including the R/W bit shifted out
                        (e.g. 0xA0 when A2=A1=A0=GND). */
  const EepromI2cChipConfig_t
      *chipConfig;    /**< Pointer to the read-only chip configuration
  ...
  uint16_t
      maxDataSize; /**< Maximum user data size in bytes for this instance.
 ...
 * @param wpPin       GPIO pin number for the Write Protect signal (e.g.
 * GPIO_PIN_5). Ignored when wpPort is NULL.
 * @param wpInverted  Write protect polarity: 0 = WP active LOW (standard),
 *                    1 = WP active HIGH (inverted, e.g. driven through NPN
 * transistor).
 * @param i2cAddr     I2C device address with the R/W bit position (e.g. 0xA0
 * for a chip with A2=A1=A0 tied to GND).
 ...
 *                    Use different offsets for multiple independent instances
 * on one physical chip.
 *
 * @note This function does not perform any I2C communication.
 * @note The handle must remain valid for the lifetime of all subsequent API
 * calls.
 */
void EEPROM_I2C_Init(EepromI2cHandle_t *handle, I2C_HandleTypeDef *hi2c,
                     GPIO_TypeDef *wpPort, uint16_t wpPin, uint8_t wpInverted,
                     uint8_t i2cAddr, EepromI2cChipType_t chipType,
                     uint16_t startAddr);

/**
 * @brief Write data to the EEPROM with CRC protection, wear leveling, and
 * write-back verification.
 *
 * The function first computes a CRC-16-Modbus checksum of the new data and
 * reads the existing header from the EEPROM. If the stored checksum and length
 * match, the actual stored bytes are compared with the new data; if they are
 * identical the write is skipped and EEPROM_I2C_ERR_IDENTICAL is returned,
 * avoiding unnecessary wear. Otherwise, the header is written first, followed
 * by the data. After writing, the header and data are read back and compared to
 * verify the write succeeded.
 *
 * @param handle      Pointer to an initialized EEPROM handle.
 * @param pData       Pointer to the source data buffer. The data is written
 * directly without an intermediate copy (zero-copy).
 * @param dataLength  Number of bytes to write. Must not exceed
 * handle->maxDataSize or EEPROM_I2C_MAX_BUFFER_SIZE.
 *
 * @return EEPROM_I2C_OK           - Data written and verified successfully.
 *         EEPROM_I2C_ERR_IDENTICAL - Write skipped; stored data is unchanged
 * (not a failure). EEPROM_I2C_ERR_SIZE      - dataLength exceeds the available
 * region or buffer limit. EEPROM_I2C_ERR_I2C       - I2C communication error
 * during write or readback. EEPROM_I2C_ERR           - handle or pData is NULL,
 * or write-back comparison failed.
 */
EepromI2cStatus_t EEPROM_I2C_Write(EepromI2cHandle_t *handle, const void *pData,
                                   uint16_t dataLength);

/**
 * @brief Read data from the EEPROM and verify the CRC checksum.
 *
 * Reads the stored header to obtain the expected data length and CRC, then
 * reads the data directly into the caller's buffer. After reading, the
 * CRC-16-Modbus checksum is computed over the received bytes and compared
 * against the stored value. If they do not match, EEPROM_I2C_ERR_CRC is
 * returned and the buffer contents should not be used.
 *
 * @param handle      Pointer to an initialized EEPROM handle.
 * @param pData       Pointer to the destination buffer. Data is read directly
 * into this buffer without an intermediate copy (zero-copy).
 * @param dataLength  Number of bytes to read. Must match the length that was
 * used during the last successful write.
 *
 ...
 */
EepromI2cStatus_t EEPROM_I2C_Read(EepromI2cHandle_t *handle, void *pData,
                                  uint16_t dataLength);
 ...
 *                        stored data length in bytes. May be NULL if not
 * needed.
 *
 ...
 */
EepromI2cStatus_t EEPROM_I2C_Validate(EepromI2cHandle_t *handle,
                                      uint16_t *pExpectedLength);
 ...
 * call, EEPROM_I2C_Read() and EEPROM_I2C_Validate() will return
 * EEPROM_I2C_ERR_SIZE (length 0 does not match any valid request) or
 * EEPROM_I2C_ERR_CRC, effectively invalidating the stored record without
 * erasing the data bytes themselves.
 *
 * @param handle  Pointer to an initialized EEPROM handle.
 *
